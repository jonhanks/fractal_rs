...
fn render_image_modulus(fractal: &mandelbrot::Data, buffer: &mut[u32], pal :&palette::PaletteData) {
    let mut offset = 0;
    let pal_len = pal.len();
    fractal.fractal_data.iter().for_each(|row| {
        row.iter().for_each(|entry| {
...
fn render_image_to_surface(fractal: &mandelbrot::Data, surface: &mut sdl2::surface::Surface, pal: &palette::Palette) {
        ...
        for event in events.poll_iter() {
            match event {
                ...
                Event::KeyDown { keycode: Some(Keycode::C), .. } => {
                    ...
                    println!("New palette type is {:?} color mode is {:?}", pal.palette_type, pal.color_mode);
                    do_redraw = true;
                }
